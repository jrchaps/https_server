void trigger_breakpoint(void);
#define assert(expression) if (!(expression)) { trigger_breakpoint(); }

#include "../jrc_modules/modules_4/sha256/sha256.c"
#include "../jrc_modules/modules_4/hkdf_sha256/hkdf_sha256.c"
#include "../jrc_modules/modules_4/curve25519/curve25519.c"
#include "../jrc_modules/modules_4/chacha20_poly1305/chacha20_poly1305.c"
#include "../jrc_modules/modules_4/ecdsa_secp256r1_sha256/ecdsa_secp256r1_sha256.c"

#define curve25519_key_length 32
#define chacha20_key_length 32
#define chacha20_nonce_length 12
#define poly1305_auth_tag_length 16

struct client_ctx {
    u32 flags;
    u8 encryption_key[chacha20_key_length];
    u8 encryption_nonce[chacha20_nonce_length];
    u64 encryption_count;
    u8 decryption_key[chacha20_key_length];
    u8 decryption_nonce[chacha20_nonce_length];
    u64 decryption_count;
    u8 handshake_secret[sha256_hash_length];
    u8 encryption_secret[sha256_hash_length];
    u8 decryption_secret[sha256_hash_length];
    u8 transcript_hash[sha256_hash_length];
};

#include "os/windows.c"

slice tls_derived_secret_info = slice_alloc(49);
u8 tls_derived_secret[sha256_hash_length];

// todo: This only works single-threaded. These should be treated as read-only
// when multi-threading. 
slice tls_handshake_encryption_secret_info = slice_alloc(54);
slice tls_handshake_decryption_secret_info = slice_alloc(54);
slice tls_encryption_secret_info = slice_alloc(54);
slice tls_decryption_secret_info = slice_alloc(54);

slice tls_key_info = slice_alloc(13);
slice tls_iv_info = slice_alloc(12);
slice tls_finished_info = slice_alloc(18);

typedef struct extension_ctx extension_ctx;
typedef struct key_ctx key_ctx;

struct extension_ctx {
    slice type;
    slice extension;
};

struct key_ctx {
    slice group;
    slice key;
};

slice decode_client_hello(slice, sha256_ctx*, u8[32]);
slice append_server_hello(slice, sha256_ctx*, u8[32]);
client_ctx handshake_cipher_keys(client_ctx, sha256_ctx, u8[32], u8[32]);
slice append_change_cipher_spec(slice);
slice append_encrypted_extensions(slice, client_ctx, sha256_ctx*);
slice append_certificate(slice, client_ctx, sha256_ctx*);
slice append_certificate_verify(slice, client_ctx, sha256_ctx*);
slice append_handshake_finished(slice, client_ctx*, sha256_ctx);

void tls_key_schedule_info(void);
slice make_tls_derived_secret_info(slice);
void make_tls_derived_secret(u8[sha256_hash_length]);
slice make_tls_handshake_encryption_secret_info(slice);
slice make_tls_handshake_decryption_secret_info(slice);
slice make_tls_decryption_secret_info(slice);
slice make_tls_encryption_secret_info(slice);
slice make_tls_key_info(slice);
slice make_tls_iv_info(slice);
slice make_tls_finished_info(slice);

slice copy_decode_error(slice);
slice copy_handshake_failure(slice);
slice copy_unexpected_message(slice);
slice copy_bad_record_mac(slice);
slice copy_decrypt_error(slice);

slice copy_u16_be(slice, u16);
slice append_u16_be(slice, u16);
slice append_random(slice, size_t);

slice tls_certificate = slice_literal(
    
);

u8 tls_certificate_key[32] = {
    0x19, 0x87, 0x11, 0x45, 0x30, 0x8A, 0x61, 0x0B, 
    0xF6, 0xC2, 0x60, 0x73, 0x89, 0xA0, 0xA1, 0x51, 
    0x83, 0x27, 0x61, 0x35, 0x4D, 0xD7, 0x4C, 0x69, 
    0xD2, 0xA8, 0xD3, 0x6D, 0xE9, 0x1B, 0x44, 0xFC
};